...
export function parseProductsCsv(csv: string): Map<string, ProductRow> {
...
export function buildProductReply(product: ProductRow): string {
  return `${product.name} (รหัสสินค้า ${product.code})\nราคา ${product.price} บาทค่ะ\nคงเหลือในสต็อก ${product.stock} ชิ้นค่ะ`;
}

interface Cache {
  data: Map<string, ProductRow>;
  fetchedAt: number;
}
...
export async function getProducts(opts: GetProductsOptions = {}): Promise<Map<string, ProductRow>> {
  ...
  try {
    ...
    const data = parseProductsCsv(csv);
