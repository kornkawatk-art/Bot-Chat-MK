...
**Excluded Group**:
A LINE group (identified by group ID, with its group name captured for display) where the bot stays completely silent — no replies, no Admin Escalations, nothing — because human staff already cover customers there directly (e.g. a group mixing customers and sales reps). Unlike Staff Member, which only suppresses escalation while keeping normal replies, an Excluded Group gets zero bot involvement at all, since a canned bot reply appearing next to a real salesperson's answer would look out of place. Registered from the Admin Group with "เพิ่มกลุ่มยกเว้น &lt;groupId&gt;" (the group ID is announced by the bot when first added to any group — see the join message), managed with "รายชื่อกลุ่มยกเว้น" (list) and "ลบกลุ่มยกเว้น &lt;เลข&gt;" (remove), the same list-then-act pattern as Staff Member and the Unanswered Question Log.
